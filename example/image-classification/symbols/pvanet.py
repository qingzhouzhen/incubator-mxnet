...
def get_symbol(num_classes, **kwargs):
    ...
    pool5 = mx.sym.Pooling(data=bn_relu, kernel=(1, 1), stride=(1, 1), pad=(0, 0), pool_type='max', name='pool5')
...
def res_crelu(data, middle_filter, num_filter, kernel, stride, pad, proj, name=None, suffix=''):
    ...
    conv1 = Conv(data=data, num_filter=middle_filter[0], kernel=(1, 1), stride=(1, 1), pad=(0, 0), name='%s%s_1_con2d' %(name, suffix))
    #crelu
    conv2 = mx.sym.Convolution(data=conv1, num_filter=middle_filter[1], kernel=kernel, stride=stride, pad=pad, name='%s%s_2_con2d' %(name, suffix))
...
def inception(data, middle_filter, num_filter, kernel, stride, proj, last, name, suffix):
    if proj:
        ...
        shortcut = data
    bn_relu = bn_and_relu(data=data, name=name, suffix='bsr')
    conv_a = Conv(data=bn_relu, num_filter=middle_filter[0], kernel=(1, 1), stride=stride, pad=(0, 0), name=name, suffix='a')

    conv_b1 = Conv(data=bn_relu, num_filter=middle_filter[1][0], kernel=(1, 1), stride=stride, pad=(0, 0), name=name, suffix='b1')
    conv_b2 = Conv(data=conv_b1, num_filter=middle_filter[1][1], kernel=kernel, stride=(1, 1), pad=(1, 1), name=name, suffix='b2')

    conv_c1 = Conv(data=bn_relu, num_filter=middle_filter[2][0], kernel=(1, 1), stride=stride, pad=(0, 0), name=name, suffix='c1')
    ...
    if stride[1] > 1:
        pool_d = mx.sym.Pooling(data=bn_relu, kernel=kernel, stride=stride, pad=(1, 1), pool_type='max', name=name+'pool')
